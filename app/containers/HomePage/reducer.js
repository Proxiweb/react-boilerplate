...
function homepageReducer(state = initialState, action) {
  switch (action.type) {
    case c.ASYNC_LOAD_DATAS_1_START:
      return update(state, {
        datas1: {
          asyncStatus: { $set: { pending: true, error: false, success: false, message: action.msgPending } },
        },
      });
    case c.ASYNC_LOAD_DATAS_1_SUCCESS:
      return update(state, {
        datas1: {
          asyncStatus: { $set: { pending: false, error: false, success: true, message: action.msgSuccess } },
    ...
    case c.ASYNC_LOAD_DATAS_1_ERROR:
      return update(state, {
        datas1: {
          asyncStatus: { $set: { pending: false, error: true, success: false, message: action.msgError } },
        },
      });
