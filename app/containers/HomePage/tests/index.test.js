...
describe('<HomePage />', () => {
  ...
  it('should render an error if loading failed', () => {
    const renderedComponent = mount(
      <IntlProvider locale="en">
        <HomePage loading={false} error={{ message: 'Loading failed!' }} />
      </IntlProvider>
    );
    expect(renderedComponent.text().indexOf('Something went wrong, please try again!')).toBeGreaterThan(-1);
  });
  ...
  it('should render the repositories if loading was successful', () => {
    ...
    const renderedComponent = shallow(<HomePage repos={repos} error={false} />);
