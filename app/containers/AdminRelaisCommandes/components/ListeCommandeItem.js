...
class ListeCommandeItem extends Component {
    ...
    return (
      <IconMenu iconButtonElement={iconButtonElement}>
        {!commande.finalisation &&
          <MenuItem
            onClick={() => this.props.pushState(`/admin/relais/${relaiId}/commandes/${commande.id}/edit`)}
          >
            Modifier
          </MenuItem>}
        {commande.finalisation &&
          <MenuItem
            onClick={() =>
              this.props.pushState(`/distributeurs/${relaiId}/factures/${commande.id}?print=true`)}
          >
            Facture
          </MenuItem>}
  ...
  render() {
    ...
    const paiementsOk =
      commande.finalisation &&
      commande.finalisation.destinataires.filter(d => d.montant > 0 && !d.paiementOk).length === 0;

    return (
      <ListItem
        primaryText={
          commande.dateCommande ? format(commande.dateCommande, "DD MMMM HH:mm") : "date indéfinie"
        }
